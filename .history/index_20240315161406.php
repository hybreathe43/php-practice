...
<br>
  ...
  //set cookies
  // $cook = setcookie("userName", "sahil", time() + 11);
  // if ($cook) {
  //   echo "SET COOKIES";
  // } else {
  //   echo "NOT SET";
  ...
  try {
    ...
    // $name = "sahil";
    // $isActive = 1;
    // $createdAt = date('Y-m-d H:i:s', strtotime('07/11/2020 9:10PM'));
    // $quer = "insert into users (email,full_name,is_active,created_at) values (?,?,?,?)"; //(:name,:email,:active,:date)
    ...
    $stmt = $db->query($quer);var_dump($user);
